        ...
        <div class="p-4 border-t border-[#3D2B1F] bg-[#24150A]">
            @auth
                ...
                <div class="flex items-center px-2 mb-4 text-left">
                    @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                        <img class="h-9 w-9 rounded-full border-2 border-[#D4AF37] object-cover"
                ...
                <div class="space-y-1">
                    <a href="{{ route('profile.show') }}"
                        class="flex items-center px-3 py-2 text-[11px] text-[#8B7355] hover:text-[#D4AF37] transition">
                    ...
                    <form method="POST" action="{{ route('logout') }}" x-data>
                        ...
                        <button type="submit" @click.prevent="$root.submit();"
                            class="w-full flex items-center px-3 py-2 text-[11px] text-[#8B7355] hover:text-red-400 transition">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path
            ...
            @else
                ...
                <div class="space-y-2 p-2">
                    <a href="{{ route('login') }}"
                        class="block w-full text-center px-4 py-2 text-xs font-bold text-[#D4AF37] border border-[#D4AF37] rounded-lg hover:bg-[#D4AF37] hover:text-[#2D1B0F] transition duration-200">
                        {{ __('ENTRAR') }}
                    </a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}"
                            class="block w-full text-center px-4 py-2 text-xs font-bold text-white bg-[#3D2B1F] rounded-lg hover:bg-[#4D3B2F] transition duration-200">
                            {{ __('REGISTRARSE') }}
                        </a>
                    @endif
                </div>
