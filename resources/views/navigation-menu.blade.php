<nav x-data="{ open: false }"
    ...
    <div class="shrink-0 flex items-center justify-center h-24 border-b border-[#3D2B1F]">
        @auth
            <a href="{{ Auth::user()->dashboardRoute() }}" class="flex flex-col items-center group">
                <x-application-mark class="block h-10 w-auto text-[#D4AF37] fill-current transition-transform group-hover:scale-110" />
    ...
    <div class="flex-grow px-4 py-6 space-y-3 overflow-y-auto custom-scrollbar flex flex-col">
        ...
        <x-nav-link href="{{ route('inicio') }}" :active="request()->routeIs('inicio')"
            ...
            <i class="fa-solid fa-house mr-3 text-lg {{ request()->routeIs('inicio') ? 'text-[#D4AF37]' : 'text-[#8B7355] group-hover:text-[#D4AF37]' }}"></i>
        ...
        @auth
            <x-nav-link href="{{ Auth::user()->dashboardRoute() }}" :active="request()->routeIs('*.dashboard')"
                ...
                <i class="fa-solid fa-chart-pie mr-3 text-lg {{ request()->routeIs('*.dashboard') ? 'text-[#D4AF37]' : 'text-[#8B7355] group-hover:text-[#D4AF37]' }}"></i>
            ...
            @if (Auth::user()->role == 'admin')
                <div class="pt-4">
                    ...
                    <x-nav-link href="{{ route('admin.servicios.index') }}" :active="request()->routeIs('admin.servicios.*')"
                        ...
                        <i class="fa-solid fa-gears mr-3 text-lg {{ request()->routeIs('admin.servicios.*') ? 'text-[#D4AF37]' : 'text-[#8B7355] group-hover:text-[#D4AF37]' }}"></i>
    ...
    <div class="p-4 border-t border-[#3D2B1F] bg-[#24150A]">
        @auth
            <div class="flex items-center px-2 mb-4 text-left">
                ...
                @else
                    <div class="h-10 w-10 rounded-xl bg-[#3D2B1F] flex items-center justify-center text-[#D4AF37] font-bold">
